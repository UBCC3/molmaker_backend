...
def mock_put_urls(monkeypatch):
    ...
    monkeypatch.setattr(
        storage, "generate_presigned_put_url", fake_generate_presigned_put_url
    )
    return calls
...
class TestConstructFetchScript:
    ...
    def test_successful_jobs_include_result_and_artifact_download_urls(
        ...
        expected = {
            "result": _url(
                "get", f"{storage.BUCKET_ROOT_DIR}/jobs/{job_id}/result.json"
            )
        }
    ...
    def test_unknown_successful_calculation_returns_only_result_url(
        self, mock_get_urls
    ):
        """
